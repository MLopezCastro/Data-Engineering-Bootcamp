# pipeline/etl_ingesta.py
# Versión corregida para tu estructura:
#   data/data_raw/      (entrada cruda)
#   data/data_clean/    (salidas limpias)
#   outputs/            (reportes de errores y calidad)
#   images/             (no se usa en el ETL)
...
# === Rutas (ajustadas a tu layout) ===
ROOT = Path(__file__).resolve().parents[1]          # Proyecto Integrador/
...
def parse_date_any(s):
    """Parsea múltiples formatos de fecha (ISO, dd/mm/yyyy, yyyy/mm/dd, compactos)."""
    return pd.to_datetime(s, errors="coerce", dayfirst=True)

_price_re = re.compile(r"[^\d,.\-]")
def parse_price(x):
    """Convierte precios con símbolos y miles/coma a float. Devuelve NaN si no es válido o negativo."""
    if pd.isna(x):
        return np.nan
    s = str(x).strip()
    ...
    s = _price_re.sub("", s)
    if "," in s and "." in s:
        s = s.replace(".", "").replace(",", ".")   # 1.200.000,50 -> 1200000.50
    elif "," in s:
        s = s.replace(",", ".")                    # 67,00 -> 67.00
    try:
        v = float(s)
    except ValueError:
        return np.nan
...
# ---------- Export (ajustado a data/data_clean y outputs/) ----------
